...
- **GH Pages (Jekyll)**:大多数现代 Jekyll 主题(minimal-mistakes、Just the Docs、Chirpy 等)支持 pandoc div。需要在 `.internal/engineering/site/_config.yml` 里启用 mermaid 插件,并把 CSS 中 `.definition` 和 `.example` 两段加进你的 site stylesheet(可直接复用 `chapter_style.css` 里相应段)。
